    ...
    async function initStudentDashboard() {
      ...
      // Verify if current student portal account has been disabled
      const curStudentId = sess.studentId || sess.student_number || sess.id;
      const curEmail = String(sess.email || '').toLowerCase().trim();
      const currentStudent = (appData.students || []).find(s => 
        (curStudentId && (s.id === curStudentId || s.student_number === curStudentId)) || 
        (curEmail && String(s.email || '').toLowerCase().trim() === curEmail)
      );
      if (currentStudent && (currentStudent.status === 'disabled' || currentStudent.status === 'inactive' || currentStudent.active === false)) {
        showToast('Account Disabled', 'Your student portal account has been disabled. Please contact the school administration.', 'error');
        setTimeout(() => {
        ...
        try {
          ...
          if (state) {
            ...
            if (activeInstallments && activeInstallments.length > 0) {
              activeInstallments = activeInstallments.filter(p => !MOCK_TEMPLATE_IDS.includes(p.id));
            }
            if (!activeInstallments || activeInstallments.length === 0) {
              try {
                const cached = localStorage.getItem('pa_installment_templates');
                if (cached) {
                  const parsed = JSON.parse(cached);
                  if (Array.isArray(parsed) && parsed.length > 0) {
                    activeInstallments = parsed.filter(p => !MOCK_TEMPLATE_IDS.includes(p.id));
                  }
                }
              } catch {}
            }
            if (!activeInstallments) activeInstallments = [];
