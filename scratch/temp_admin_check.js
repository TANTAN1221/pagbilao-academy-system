  ...
  <script>
    tailwind.config = { theme: { extend: { colors: { academy: { navy: '#1E3A8A', blue: '#2563EB', soft: '#F5F8FF', border: '#DDE8F8', text: '#0F2A5F', muted: '#6B7A90', light: '#EEF5FF' } }, boxShadow: { soft: '0 20px 50px rgba(30,58,138,.10)', card: '0 10px 30px rgba(30,58,138,.08)' } } } };
  </script>
</head>

<body class="min-h-screen bg-[#F7FAFF] text-academy-text">
  <div id="toast"
    class="fixed top-5 right-5 z-[200] hidden max-w-sm rounded-2xl border border-academy-border bg-white shadow-2xl p-4">
    <div class="flex gap-3">
      ...
      <div>
        ...
        <p id="toastMessage" class="mt-1 text-xs text-academy-muted leading-5">Action completed.</p>
      </div>
    </div>
  </div>

  <aside class="fixed inset-y-0 left-0 z-40 w-[250px] bg-academy-navy text-white hidden lg:flex flex-col border-r border-white/10 shadow-xl">
    <div class="px-5 py-5 flex items-center gap-3 border-b border-white/10">
      <img src="assets/logo.png" alt="Pagbilao Academy Inc. Logo"
        class="w-12 h-12 rounded-full bg-white object-contain border border-white/30 p-0.5 shadow-sm">
      <div>
        <h1 class="text-sm font-extrabold leading-tight">Pagbilao Academy</h1>
        <p class="text-xs text-blue-200">Admin Portal</p>
      </div>
    </div>
    <nav id="sideNav" class="flex-1 px-3 py-5 space-y-2 text-sm overflow-y-auto"></nav>
  ...
  <div class="lg:pl-[250px] min-h-screen">
    <header
      class="h-16 bg-white border-b border-academy-border flex items-center justify-between px-4 sm:px-6 sticky top-0 z-30 shadow-xs">
      <div class="flex items-center gap-3">
        ...
        <div>
          <h2 id="pageTitle" class="text-lg sm:text-xl font-extrabold text-academy-navy">Payment Dashboard</h2>
          <p id="pageSubtitle" class="hidden sm:block text-xs text-academy-muted">Monitor billing, collections, and clearance workflow</p>
        </div>
      </div>
      <div class="flex items-center gap-3">
        <button onclick="clearAllTransactions()"
          class="hidden sm:inline-flex px-4 py-2 rounded-xl bg-red-50 text-red-600 border border-red-200/60 text-xs font-extrabold hover:bg-red-100 transition">
          Clear All Transactions
        </button>
        <button onclick="downloadBackup()"
          class="hidden sm:inline-flex px-4 py-2 rounded-xl bg-academy-soft border border-academy-border text-academy-navy text-xs font-extrabold">
          Backup JSON
        </button>
        <div id="currentUserBadge" class="flex items-center gap-2.5 pl-2 border-l border-academy-border">
          <div id="userAvatar" class="w-9 h-9 rounded-full bg-academy-navy text-white grid place-items-center text-xs font-bold border-2 border-blue-100 shadow-xs">
            AD
          </div>
          <div class="hidden md:block text-left text-xs leading-tight">
            <p id="userNameDisplay" class="font-bold text-academy-navy">Admin Accountant</p>
            <p id="userEmailDisplay" class="text-[11px] text-academy-muted">admin@pagbilao.edu.ph</p>
          </div>
        </div>
      </div>
    </header>
    ...
    <main class="p-4 sm:p-6 space-y-6">
      <section id="dashboardPage" class="page-section space-y-5">
        <div class="grid sm:grid-cols-2 xl:grid-cols-4 gap-4">
          <div class="rounded-2xl bg-academy-navy text-white p-5 shadow-card">
            <p class="text-sm text-blue-100">Total Students</p>
            <h3 id="totalStudents" class="mt-4 text-3xl font-extrabold">0</h3>
          </div>
          <div class="rounded-2xl bg-green-50 p-5 border border-green-100">
            <p class="text-sm text-green-700">Total Collected</p>
            <h3 id="totalCollected" class="mt-4 text-3xl font-extrabold text-green-700">₱0</h3>
          </div>
          <div class="rounded-2xl bg-red-50 p-5 border border-red-100">
            <p class="text-sm text-red-700">Outstanding Balance</p>
            <h3 id="totalOutstanding" class="mt-4 text-3xl font-extrabold text-red-700">₱0</h3>
          </div>
          <div class="rounded-2xl bg-sky-50 p-5 border border-sky-100">
            <p class="text-sm text-academy-navy">Voucher Discounts</p>
            <h3 id="voucherTotal" class="mt-4 text-3xl font-extrabold text-academy-navy">₱0</h3>
          </div>
        </div>
        <section class="bg-white border border-academy-border rounded-2xl shadow-sm overflow-hidden">
          <div class="p-5 flex flex-col lg:flex-row gap-3 lg:items-center justify-between">
            <div>
              <h3 class="font-extrabold text-academy-navy">Student Accounts</h3>
              <p class="text-sm text-academy-muted">JHS and SHS balances with vouchers and partial payments.</p>
            </div>
            <div class="grid sm:grid-cols-3 gap-2"><input id="studentSearch" oninput="renderStudents()"
                placeholder="Search student..." class="px-4 py-3 rounded-xl border border-academy-border"><select
                id="levelFilter" onchange="renderStudents()"
                class="px-4 py-3 rounded-xl border border-academy-border bg-white">
                <option value="all">All levels</option>
                <option>JHS</option>
                <option>SHS</option>
              </select><select id="balanceFilter" onchange="renderStudents()"
                class="px-4 py-3 rounded-xl border border-academy-border bg-white">
          ...
          <div class="overflow-x-auto">
            <table class="w-full min-w-[1080px] text-sm">
              <thead class="bg-academy-navy text-white">
                <tr>
                  ...
                  <th class="text-left px-4 py-3">Total Fees</th>
                  <th class="text-left px-4 py-3">Paid</th>
                  <th class="text-left px-4 py-3">Balance</th>
                  <th class="text-left px-4 py-3">Due Status</th>
      ...
      <section id="feesPage" class="page-section hidden space-y-5">
        <div class="bg-white border border-academy-border rounded-2xl p-5 shadow-sm">
          <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
            <div>
              <h3 class="text-xl font-extrabold text-academy-navy">Tuition, Fees, and Vouchers</h3>
              <p class="text-sm text-academy-muted mt-1">Separate fee setups for JHS and SHS, with ESC Voucher and SHS Voucher discounts.</p>
            </div>
            <div class="flex flex-wrap items-center gap-2">
              <button onclick="clearAllFeesAndVouchers()" class="px-4 py-3 rounded-xl bg-red-50 text-red-600 border border-red-200/60 text-xs font-bold hover:bg-red-100 transition">
                Clear All Fees & Vouchers
              </button>
              <button onclick="applyFeeSetup()" class="px-5 py-3 rounded-xl bg-green-600 text-white font-bold">
                Apply Fee Setup to Students
              </button>
            </div>
          </div>
        </div>
        <div class="grid xl:grid-cols-2 gap-5">
          <section class="bg-white border border-academy-border rounded-2xl p-5 shadow-sm">
            <div class="flex justify-between items-center mb-4">
              <h4 class="font-extrabold text-academy-navy">Fee Structures</h4><button onclick="openFeeItemModal()"
                class="px-4 py-2 rounded-xl bg-academy-navy text-white text-xs font-bold">Add Fee</button>
            </div>
            <div id="feeStructureList" class="space-y-4"></div>
          </section>
          <section class="bg-white border border-academy-border rounded-2xl p-5 shadow-sm">
            <div class="flex justify-between items-center mb-4">
              <h4 class="font-extrabold text-academy-navy">Voucher Discounts</h4><button onclick="openVoucherModal()"
                class="px-4 py-2 rounded-xl bg-academy-navy text-white text-xs font-bold">Add Voucher</button>
            </div>
            <div id="voucherList" class="space-y-3"></div>
            <div
              class="mt-5 p-4 bg-academy-soft border border-academy-border rounded-2xl text-sm leading-7 text-academy-muted">
              <strong class="text-academy-navy">Recommended rule:</strong> voucher discount should reduce total
              assessment first, then partial payments are applied against the remaining balance using oldest-due-first
              allocation.</div>
          </section>
        </div>
      </section>

      <section id="dueDatesPage" class="page-section hidden space-y-5">
        <div class="bg-white border border-academy-border rounded-2xl p-5 shadow-sm">
          <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
            <div>
              <h3 class="text-xl font-extrabold text-academy-navy">Due Dates and Payment Schedule</h3>
              <p class="text-sm text-academy-muted mt-1">Configure custom due dates and descriptions for student payment tracking.</p>
            </div>
            <button onclick="sendBulkDueDateReminders()" class="px-4 py-2.5 rounded-xl bg-amber-500 hover:bg-amber-600 text-white font-extrabold text-xs transition flex items-center gap-1.5 shadow-sm shrink-0">
              <span class="material-symbols-rounded text-base">notifications_active</span> Send Due Date Reminders
            </button>
          </div>
        </div>
        <div class="space-y-5">
          <section class="bg-white border border-academy-border rounded-2xl p-6 shadow-sm">
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 pb-4 border-b border-academy-border">
              <div>
                <h4 class="font-extrabold text-academy-navy text-base">Due Date Schedules</h4>
                <p class="text-xs text-academy-muted mt-0.5">Active payment milestones, due dates & installment descriptions configured for students</p>
              </div>
              <div class="flex items-center gap-2">
                <button onclick="clearAllDueDates()" class="px-3 py-2 rounded-xl bg-red-50 text-red-600 border border-red-100 text-xs font-bold hover:bg-red-100 transition">Clear All</button>
                <button onclick="openInstallmentModal()" class="px-4 py-2 rounded-xl bg-academy-navy text-white text-xs font-bold hover:bg-blue-900 transition flex items-center gap-1.5 shadow-sm">
                  <span class="material-symbols-rounded text-base">add</span> Add Due Date
                </button>
              </div>
            </div>
            <div id="installmentTemplateList" class="grid sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-4 gap-4"></div>
          </section>
        </div>
      </section>

      <section id="clearancePage" class="page-section hidden space-y-5">
        <div class="bg-white border border-academy-border rounded-2xl p-5 shadow-sm">
          <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
            <div>
              <h3 class="text-xl font-extrabold text-academy-navy">Clearance Monitoring</h3>
              <p class="text-sm text-academy-muted mt-1">Teachers can have many grade/section/subject assignments.
            </div>
            <button onclick="refreshClearanceApprovals()"
              class="px-5 py-3 rounded-xl bg-academy-navy text-white font-bold hover:bg-blue-900 transition flex items-center gap-1.5">
              <span class="material-symbols-rounded text-sm">refresh</span>
              Refresh Approval Requirements
            </button>
          </div>
        </div>
        <section class="bg-white border border-academy-border rounded-2xl shadow-sm overflow-hidden">
            ...
            <table class="w-full min-w-[1150px] text-sm">
              <thead class="bg-academy-navy text-white">
                <tr>
                  <th class="text-left px-4 py-3">Student</th>
                  <th class="text-left px-4 py-3">Class</th>
                  <th class="text-left px-4 py-3 whitespace-nowrap min-w-[175px]">Teachers Needed</th>
                  <th class="text-left px-4 py-3">Office Clearance</th>
                  <th class="text-left px-4 py-3">Principal</th>
                  <th class="text-left px-4 py-3">Accounting/Registrar</th>
                  <th class="text-left px-4 py-3">Certificate</th>
                  <th class="text-left px-4 py-3">Actions</th>
                </tr>
              </thead>
              <tbody id="clearanceBody" class="divide-y divide-slate-100"></tbody>
            </table>
          </div>
        </section>
      </section>

      <section id="accountsPage" class="page-section hidden space-y-5">
        <!-- Accounts Page Header with Segmented Navigation Sub-tabs -->
        <div class="bg-white border border-academy-border rounded-2xl p-5 shadow-sm space-y-4">
          <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
            <div>
              <div class="flex items-center gap-2">
                <h3 class="text-xl font-extrabold text-academy-navy">Accounts & Access Management</h3>
                <span class="px-2.5 py-0.5 rounded-full text-xs font-extrabold bg-blue-50 text-academy-blue border border-blue-100">Separated Portals</span>
              </div>
              <p class="text-sm text-academy-muted mt-1">Manage staff clearance heads and student portal access in separate, dedicated sections to ensure clear role boundaries.</p>
            </div>
          </div>

          <!-- Segmented Navigation Tabs -->
          <div class="flex flex-wrap items-center gap-2 p-1.5 bg-slate-100 rounded-2xl border border-academy-border w-fit">
            <button id="tabStaffAccounts" type="button" onclick="switchAccountsTab('staff')"
              class="px-5 py-2.5 rounded-xl font-extrabold text-sm transition flex items-center gap-2 shadow-xs bg-academy-navy text-white">
              <span class="material-symbols-rounded text-lg">badge</span>
              Staff & Clearance Heads
              <span id="badgeStaffCount" class="px-2 py-0.5 rounded-full text-xs bg-white/20 text-white font-bold">0</span>
            </button>
            <button id="tabStudentAccounts" type="button" onclick="switchAccountsTab('students')"
              class="px-5 py-2.5 rounded-xl font-extrabold text-sm transition flex items-center gap-2 text-slate-600 hover:text-academy-navy hover:bg-white/60">
              <span class="material-symbols-rounded text-lg">school</span>
              Student Portal Accounts
              <span id="badgeStudentCount" class="px-2 py-0.5 rounded-full text-xs bg-slate-200 text-slate-700 font-bold">0</span>
            </button>
          </div>
        </div>

        <!-- 1. STAFF & CLEARANCE HEADS VIEW -->
        <div id="staffAccountsSection" class="space-y-5">
          <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-blue-50/70 border border-blue-100/80 rounded-2xl p-4">
            <div class="flex items-center gap-3">
              <div class="w-10 h-10 rounded-xl bg-academy-navy text-white flex items-center justify-center shrink-0 shadow-xs">
                <span class="material-symbols-rounded text-xl">admin_panel_settings</span>
              </div>
              <div>
                <h4 class="font-extrabold text-academy-navy text-sm">Clearance Heads & Faculty Access</h4>
                <p class="text-xs text-academy-muted">Assign teachers to grade/section/subject scopes and configure administrative signing roles.</p>
              </div>
            </div>
            <button type="button" onclick="openAccountModal()"
              class="px-4 py-2.5 rounded-xl bg-academy-navy hover:bg-blue-900 text-white font-extrabold text-xs shadow-sm flex items-center gap-1.5 shrink-0 transition">
              <span class="material-symbols-rounded text-base">person_add</span> Create Staff Account
            </button>
          </div>

          <section class="grid sm:grid-cols-3 gap-4">
            <div class="rounded-2xl bg-white border border-academy-border p-5 shadow-xs">
              <p class="text-xs font-bold text-academy-muted uppercase tracking-wider">Active Staff Accounts</p>
              <h3 id="activeAccounts" class="text-3xl font-extrabold text-academy-navy mt-2">0</h3>
            </div>
            <div class="rounded-2xl bg-green-50 border border-green-100 p-5 shadow-xs">
              <p class="text-xs font-bold text-green-700 uppercase tracking-wider">Teacher Assignments</p>
              <h3 id="teacherAssignments" class="text-3xl font-extrabold text-green-700 mt-2">0</h3>
            </div>
            <div class="rounded-2xl bg-sky-50 border border-sky-100 p-5 shadow-xs">
              <p class="text-xs font-bold text-academy-navy uppercase tracking-wider">Office & Clearance Heads</p>
              <h3 id="allStudentHeads" class="text-3xl font-extrabold text-academy-navy mt-2">0</h3>
            </div>
          </section>
          ...
          <section class="bg-white border border-academy-border rounded-2xl shadow-sm overflow-hidden">
            <div class="p-4 border-b border-slate-100 flex items-center justify-between">
              <div>
                <h4 class="font-extrabold text-academy-navy text-sm">Staff Account Roster</h4>
                <p class="text-xs text-academy-muted">Deactivating a staff account revokes clearance signing and faculty access.</p>
              </div>
            </div>
            <div class="overflow-x-auto">
              <table class="w-full min-w-[1120px] text-sm">
                <thead class="bg-academy-navy text-white">
                  <tr>
                    ...
                    <th class="text-left px-4 py-3">Assignments / Scope</th>
        ...
        <!-- 2. STUDENT PORTAL ACCOUNTS VIEW -->
        <div id="studentAccountsSection" class="hidden space-y-5">
          <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-amber-50/60 border border-amber-200/60 rounded-2xl p-4">
            <div class="flex items-center gap-3">
              ...
              <div>
                <h4 class="font-extrabold text-academy-navy text-sm">Enrolled Student Portal Accounts</h4>
                <p class="text-xs text-academy-muted">Manage student login credentials and portal access. Disabling an account prevents the student from logging in without affecting staff clearance permissions.</p>
              </div>
            </div>
            <button type="button" onclick="openStudentAccountModal()"
              class="px-4 py-2.5 rounded-xl bg-academy-navy hover:bg-blue-900 text-white font-extrabold text-xs shadow-sm flex items-center gap-1.5 shrink-0 transition">
              <span class="material-symbols-rounded text-base">person_add</span> Register / Add Student Account
            </button>
          </div>
          ...
          <section class="grid sm:grid-cols-3 gap-4">
            <div class="rounded-2xl bg-white border border-academy-border p-5 shadow-xs">
              <div class="flex items-center justify-between">
                <p class="text-xs font-bold text-academy-muted uppercase tracking-wider">Total Enrolled Students</p>
                <span class="material-symbols-rounded text-academy-navy">school</span>
              </div>
              <h3 id="totalStudentAccountsCount" class="text-3xl font-extrabold text-academy-navy mt-2">0</h3>
            </div>
            <div class="rounded-2xl bg-emerald-50 border border-emerald-100 p-5 shadow-xs">
              <div class="flex items-center justify-between">
                <p class="text-xs font-bold text-emerald-700 uppercase tracking-wider">Active Portal Access</p>
                <span class="material-symbols-rounded text-emerald-600">check_circle</span>
              </div>
              <h3 id="activeStudentAccountsCount" class="text-3xl font-extrabold text-emerald-700 mt-2">0</h3>
            </div>
            <div class="rounded-2xl bg-rose-50 border border-rose-100 p-5 shadow-xs">
              <div class="flex items-center justify-between">
                <p class="text-xs font-bold text-rose-700 uppercase tracking-wider">Disabled / Suspended Access</p>
                <span class="material-symbols-rounded text-rose-600">block</span>
              </div>
              <h3 id="disabledStudentAccountsCount" class="text-3xl font-extrabold text-rose-700 mt-2">0</h3>
            </div>
          </section>

          <!-- Search & Filter Controls -->
          <div class="bg-white border border-academy-border rounded-2xl p-4 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-3">
            <div class="relative flex-1">
              <span class="material-symbols-rounded absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400 text-lg">search</span>
              <input id="studentAccountSearchInput" type="text" placeholder="Search student by name, student number, or email..."
                oninput="renderStudentAccounts()"
                class="w-full pl-10 pr-4 py-2.5 rounded-xl border border-academy-border text-xs font-medium focus:outline-none focus:ring-2 focus:ring-academy-blue/20">
            </div>
            <div class="flex flex-wrap items-center gap-2">
              <select id="studentAccountGradeFilter" onchange="renderStudentAccounts()"
                class="px-3 py-2 rounded-xl border border-academy-border text-xs font-bold bg-white text-academy-navy">
              ...
              <select id="studentAccountStatusFilter" onchange="renderStudentAccounts()"
                class="px-3 py-2 rounded-xl border border-academy-border text-xs font-bold bg-white text-academy-navy">
                <option value="all">All Statuses</option>
                <option value="active">Active Only</option>
                <option value="disabled">Disabled Only</option>
              </select>
              <button type="button" onclick="resetStudentAccountFilters()"
                class="px-3 py-2 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-600 font-bold text-xs transition">
                Reset
              </button>
            </div>
          </div>
              ...
              <table class="w-full min-w-[1050px] text-sm">
                <thead class="bg-academy-navy text-white">
                  <tr>
                    ...
                    <th class="text-left px-4 py-3">Grade & Section</th>
                    <th class="text-left px-4 py-3">Portal Login Email</th>
                    <th class="text-left px-4 py-3">Portal Access Status</th>
                    <th class="text-left px-4 py-3">Actions</th>
                  </tr>
                </thead>
                <tbody id="studentAccountsBody" class="divide-y divide-slate-100"></tbody>
              </table>
            </div>
          </section>
        </div>
      ...
      <section id="analyticsPage" class="page-section hidden space-y-5">
        <div class="bg-white border border-academy-border rounded-2xl p-5 shadow-sm">
          <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
            <div>
              <h3 class="text-xl font-extrabold text-academy-navy">Accounting Analytics & Reports</h3>
              <p class="text-sm text-academy-muted mt-1">Reports that help the institution track collections, balances,
                voucher impact, overdue accounts, and clearance blocks.</p>
            </div>
            <div class="flex gap-2"><button onclick="exportCollectionReport()"
                class="px-4 py-3 rounded-xl bg-academy-navy text-white font-bold text-sm">Export Collections
                CSV</button><button onclick="exportOutstandingReport()"
                class="px-4 py-3 rounded-xl bg-white border border-academy-border text-academy-navy font-bold text-sm">Export
                Outstanding CSV</button></div>
          </div>
        </div>

        <!-- GEMINI AI FINANCIAL ANALYSIS & DATA INTERPRETATION -->
        <section class="bg-gradient-to-br from-slate-900 via-academy-navy to-blue-950 text-white rounded-2xl p-6 shadow-xl border border-blue-800/40 space-y-4">
          <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-white/10 pb-4">
            <div class="flex items-center gap-3">
              <div class="w-11 h-11 rounded-2xl bg-blue-500/20 text-blue-300 border border-blue-400/30 flex items-center justify-center shrink-0">
                <span class="material-symbols-rounded text-2xl">auto_awesome</span>
              </div>
              <div>
                <div class="flex items-center gap-2">
                  <h4 class="text-base font-extrabold tracking-tight">Gemini AI Executive Financial Analysis</h4>
                  <span class="px-2 py-0.5 rounded-full bg-blue-500/30 text-blue-200 border border-blue-400/30 text-[10px] font-extrabold uppercase">AI Insights</span>
                </div>
                <p class="text-xs text-blue-200 mt-0.5">Automated financial health rating, collection trend insights, and risk recommendations</p>
              </div>
            </div>

            <div class="flex items-center gap-2">
              <button onclick="generateGeminiAiAnalyticsInterpretation()" id="btnGenerateAiReport"
                class="px-5 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-500 text-white font-extrabold text-xs shadow-md transition flex items-center gap-1.5 shrink-0">
          ...
          <!-- AI Output Box -->
          <div id="geminiAiReportBox" class="bg-white/5 border border-white/10 rounded-xl p-4.5 text-xs space-y-2 leading-relaxed">
            <div class="flex items-center justify-between text-blue-200 text-xs">
              <span class="font-bold flex items-center gap-1.5">
                <span class="material-symbols-rounded text-sm text-amber-400">info</span>
                Ready for AI Financial Interpretation
              </span>
              <span>Gemini 1.5 / 2.0 Flash Integration</span>
            </div>
            <p class="text-slate-300 text-xs leading-5">
              Click <strong>Analyze Data</strong> to generate an instant executive financial summary, uncollected risk assessment, and recommended accounting action plan based on live school metrics.
            </p>
          </div>
        </section>
        ...
        <div class="grid xl:grid-cols-2 gap-5 mt-5">
          <section class="bg-white border border-academy-border rounded-2xl p-5 shadow-sm">
            <div class="flex items-center justify-between mb-4">
              <div>
                <h4 class="font-extrabold text-academy-navy text-base">Collection Distribution Chart</h4>
                <p class="text-xs text-academy-muted">Collected Payments vs Outstanding Balance vs Voucher Discounts</p>
              </div>
              <span class="material-symbols-rounded text-academy-blue">pie_chart</span>
            </div>
          ...
          <section class="bg-white border border-academy-border rounded-2xl p-5 shadow-sm">
            <div class="flex items-center justify-between mb-4">
              <div>
                <h4 class="font-extrabold text-academy-navy text-base">JHS vs SHS Fee Assessment & Collection</h4>
                <p class="text-xs text-academy-muted">Comparing total assessed tuition fees against actual collected payments per level</p>
              </div>
              <span class="material-symbols-rounded text-academy-blue">bar_chart</span>
            </div>
            <div class="h-64 relative">
              <canvas id="levelComparisonBarChart"></canvas>
            </div>
          </section>
        </div>

        <section class="grid md:grid-cols-4 gap-4 mt-5" id="analyticsCards"></section>
        <div class="grid xl:grid-cols-2 gap-5">
          <section class="bg-white border border-academy-border rounded-2xl p-5 shadow-sm">
            <h4 class="font-extrabold text-academy-navy mb-4">Collection Progress by Level</h4>
            <div id="collectionByLevel" class="space-y-4"></div>
          </section>
          <section class="bg-white border border-academy-border rounded-2xl p-5 shadow-sm">
            <h4 class="font-extrabold text-academy-navy mb-4">Overdue / At-Risk Accounts</h4>
            <div id="overdueList" class="space-y-3"></div>
          </section>
        </div>
        <section class="bg-white border border-academy-border rounded-2xl shadow-sm overflow-hidden mt-5">
          <div class="p-5 border-b border-academy-border flex justify-between items-center">
            <div>
              <h4 class="font-extrabold text-academy-navy">Recent Payment Transactions & Receipts</h4>
              <p class="text-xs text-academy-muted">Real-time ledger of online (QR Ph, PayMongo) and in-person payments</p>
            </div>
            <span id="adminTxCountBadge" class="px-2.5 py-1 rounded-full bg-emerald-50 text-emerald-700 font-extrabold text-xs">0 Transactions</span>
          </div>
          <div class="overflow-x-auto">
            <table class="w-full min-w-[900px] text-sm">
              <thead class="bg-slate-900 text-white">
                <tr>
                  <th class="text-left px-4 py-3">Ref / OR No.</th>
                  <th class="text-left px-4 py-3">Student</th>
                  <th class="text-left px-4 py-3">Payment Channel</th>
                  <th class="text-left px-4 py-3">Date</th>
                  <th class="text-left px-4 py-3">Fee Breakdown</th>
                  <th class="text-right px-4 py-3">Amount</th>
                </tr>
              </thead>
              <tbody id="adminPaymentsBody" class="divide-y divide-slate-100"></tbody>
            </table>
          </div>
        </section>
      </section>

      <section id="certificatesPage" class="page-section hidden space-y-5">
        <div class="bg-white border border-academy-border rounded-2xl p-5 shadow-sm">
          <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
            <div>
              <h3 class="text-xl font-extrabold text-academy-navy">Clearance Certificates</h3>
              <p class="text-sm text-academy-muted mt-1">After full clearance, students can request a certificate and
                save it as PDF.</p>
            </div>
          </div>
        </div>
            ...
            <table class="w-full min-w-[900px] text-sm">
              <thead class="bg-academy-navy text-white">
                <tr>
                  <th class="text-left px-4 py-3">Student</th>
                  <th class="text-left px-4 py-3">Class</th>
                  <th class="text-left px-4 py-3">Clearance Status</th>
                  <th class="text-left px-4 py-3">Certificate Request</th>
                  <th class="text-left px-4 py-3">Action</th>
                </tr>
              </thead>
              <tbody id="certificateBody" class="divide-y divide-slate-100"></tbody>
            </table>
          </div>
        </section>
      </section>
      ...
      <section id="settingsPage" class="page-section hidden">
        <div class="grid lg:grid-cols-2 gap-5">
          <section class="bg-white border border-academy-border rounded-2xl p-5 shadow-sm">
            <h3 class="text-xl font-extrabold text-academy-navy">System Settings</h3>
            <p class="text-sm text-academy-muted mt-1">Update system organization details and school year configuration.</p>
            <form id="settingsForm" class="mt-6 space-y-4">
              <div><label class="block text-sm font-bold mb-2">School Name</label><input id="schoolNameInput"
                  value="Pagbilao Academy Inc." class="w-full px-4 py-3 rounded-xl border border-academy-border"></div>
              <div><label class="block text-sm font-bold mb-2">School Year</label><input id="schoolYearInput"
                  value="2026-2027" class="w-full px-4 py-3 rounded-xl border border-academy-border"></div><button
                class="px-5 py-3 rounded-xl bg-academy-navy text-white font-bold">Save Settings</button>
            </form>
          </section>
        </div>
      </section>
  ...
  <div id="formModal"
    class="fixed inset-0 z-[150] hidden items-center justify-center bg-slate-950/50 backdrop-blur-sm px-4">
    <div
      class="w-full max-w-2xl bg-white rounded-[2rem] border border-academy-border shadow-2xl p-6 max-h-[90vh] overflow-y-auto">
      <div class="flex justify-between gap-4 mb-5">
        <div>
          <p id="formModalLabel" class="text-sm font-extrabold text-academy-blue uppercase tracking-widest">Record</p>
          <h2 id="formModalTitle" class="mt-1 text-2xl font-extrabold text-academy-navy">Add Record</h2>
        </div><button onclick="closeFormModal()"
          class="w-10 h-10 rounded-xl bg-slate-50 border border-academy-border text-academy-muted hover:text-red-600">×</button>
      </div>
  ...
  <div id="confirmModal"
    class="fixed inset-0 z-[250] hidden items-center justify-center bg-slate-950/60 backdrop-blur-md px-4">
    <div
      class="w-full max-w-md bg-white rounded-[2rem] border border-academy-border shadow-2xl p-6 text-center animate-in fade-in zoom-in duration-200">
      ...
      <h3 id="confirmModalTitle" class="text-xl font-extrabold text-academy-navy mb-2">Confirm Action</h3>
      <p id="confirmModalMessage" class="text-xs font-semibold text-academy-muted mb-6 leading-relaxed">Are you sure you want to perform this action?</p>
      <div class="flex items-center gap-3">
        <button id="confirmModalCancelBtn" class="w-1/2 py-3 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 font-extrabold text-xs transition">
          Cancel
        </button>
        <button id="confirmModalActionBtn" class="w-1/2 py-3 rounded-xl bg-red-600 hover:bg-red-700 text-white font-extrabold text-xs shadow-md transition">
          Confirm Delete
        </button>
      </div>
    </div>
  ...
  <script>
    ...
    const navItems = [
      ['dashboardPage', 'dashboard', 'Payment Dashboard', 'Monitor student balances'], ['feesPage', 'payments', 'Tuition & Vouchers', 'JHS/SHS fees and discounts'], ['dueDatesPage', 'calendar_month', 'Due Dates', 'Installments and partial payments'], ['clearancePage', 'fact_check', 'Clearance Monitoring', 'Approval workflow'], ['accountsPage', 'manage_accounts', 'Accounts & Access', 'Staff & student accounts'], ['analyticsPage', 'monitoring', 'Analytics & Reports', 'Accounting reports'], ['certificatesPage', 'workspace_premium', 'Certificates', 'Clearance certificate requests'], ['settingsPage', 'settings', 'Settings', 'System configuration']
    ];
    const gradeSections = {
      JHS: {
    ...
    function ensurePrototypeData(data = appData) {
      ...
      if (!data.installmentTemplate || !data.installmentTemplate.length) {
        try {
          ...
          if (cached) {
            ...
            if (Array.isArray(parsed) && parsed.length > 0) {
              ...
              if (clean.length > 0) {
                data.installmentTemplate = clean;
              } else {
        ...
        if (!data.installmentTemplate) {
          data.installmentTemplate = [];
        }
      } else {
        data.installmentTemplate = (data.installmentTemplate || []).filter(p => !MOCK_TEMPLATE_IDS.includes(p.id));
      }
    ...
    function renderAccounts() { 
      const staffActive = appData.accounts.filter(a => a.active).length;
      activeAccounts.textContent = staffActive; 
      teacherAssignments.textContent = appData.accounts.reduce((n, a) => n + (a.assignments || []).length, 0); 
      allStudentHeads.textContent = appData.accounts.filter(a => ['guidance_head', 'prefect_head', 'librarian_head', 'principal', 'accounting_admin', 'registrar'].includes(a.role)).length; 
      const badgeStaff = document.getElementById('badgeStaffCount');
      if (badgeStaff) badgeStaff.textContent = appData.accounts.length;

      accountsBody.innerHTML = appData.accounts.map(a => `<tr class="hover:bg-academy-soft"><td class="px-4 py-3 font-extrabold text-academy-navy">${a.name}</td><td class="px-4 py-3">${a.email}</td><td class="px-4 py-3">${roleLabel(a.role)}</td><td class="px-4 py-3">${assignmentSummary(a)}</td><td class="px-4 py-3">${pill(a.active ? 'Active' : 'Inactive', a.active ? 'green' : 'gray')}</td><td class="px-4 py-3"><div class="flex gap-2"><button onclick="openAccountModal('${a.id}')" class="px-3 py-1.5 rounded-lg bg-sky-50 text-academy-navy font-bold text-xs hover:bg-sky-100 transition">Edit</button><button onclick="toggleAccount('${a.id}')" class="px-3 py-1.5 rounded-lg bg-amber-50 text-amber-700 font-bold text-xs hover:bg-amber-100 transition">${a.active ? 'Deactivate Staff' : 'Activate Staff'}</button><button onclick="deleteAccount('${a.id}')" class="px-3 py-1.5 rounded-lg bg-red-50 text-red-600 font-bold text-xs hover:bg-red-100 transition">Delete</button></div></td></tr>`).join('');
      
      if (typeof renderStudentAccounts === 'function') renderStudentAccounts();
    }
...
4. <h5 class="font-bold text-white text-xs mt-3 mb-1 flex items-center gap-1.5"><span class="material-symbols-rounded text-sm text-yellow-400">lightbulb</span> Recommended Accounting Actions:</h5>
   <ol class="list-decimal pl-4 space-y-1 text-blue-200">
    ...
    // ==========================================
    // SEPARATED STUDENT ACCOUNT MANAGEMENT
    // ==========================================
    let currentAccountsTab = 'staff';

    function switchAccountsTab(tab) {
      currentAccountsTab = tab;
      const tabStaff = document.getElementById('tabStaffAccounts');
      const tabStudent = document.getElementById('tabStudentAccounts');
      const staffSection = document.getElementById('staffAccountsSection');
      const studentSection = document.getElementById('studentAccountsSection');
      const badgeStaff = document.getElementById('badgeStaffCount');
      const badgeStudent = document.getElementById('badgeStudentCount');

      if (tab === 'students') {
        if (tabStudent) tabStudent.className = 'px-5 py-2.5 rounded-xl font-extrabold text-sm transition flex items-center gap-2 shadow-xs bg-academy-navy text-white';
        if (tabStaff) tabStaff.className = 'px-5 py-2.5 rounded-xl font-extrabold text-sm transition flex items-center gap-2 text-slate-600 hover:text-academy-navy hover:bg-white/60';
        if (badgeStudent) badgeStudent.className = 'px-2 py-0.5 rounded-full text-xs bg-white/20 text-white font-bold';
        if (badgeStaff) badgeStaff.className = 'px-2 py-0.5 rounded-full text-xs bg-slate-200 text-slate-700 font-bold';
        ...
        if (tabStaff) tabStaff.className = 'px-5 py-2.5 rounded-xl font-extrabold text-sm transition flex items-center gap-2 shadow-xs bg-academy-navy text-white';
        if (tabStudent) tabStudent.className = 'px-5 py-2.5 rounded-xl font-extrabold text-sm transition flex items-center gap-2 text-slate-600 hover:text-academy-navy hover:bg-white/60';
        if (badgeStaff) badgeStaff.className = 'px-2 py-0.5 rounded-full text-xs bg-white/20 text-white font-bold';
        if (badgeStudent) badgeStudent.className = 'px-2 py-0.5 rounded-full text-xs bg-slate-200 text-slate-700 font-bold';
        if (staffSection) staffSection.classList.remove('hidden');
        if (studentSection) studentSection.classList.add('hidden');
      }
    }

    function resetStudentAccountFilters() {
      const searchInput = document.getElementById('studentAccountSearchInput');
      const gradeFilter = document.getElementById('studentAccountGradeFilter');
      const statusFilter = document.getElementById('studentAccountStatusFilter');
    ...
    function getAllRegisteredStudentsList() {
      ...
      try {
        ...
        localReg.forEach(lr => {
          if (lr && (lr.role === 'student' || lr.studentId)) {
            const sid = lr.studentId || lr.id;
            const lemail = String(lr.email || '').toLowerCase().trim();
            const exists = list.some(s => 
              (sid && s.id === sid) || 
              (lemail && String(s.email || '').toLowerCase().trim() === lemail)
            );
            if (!exists) {
              list.push({
                id: sid || `STU-${Date.now()}`,
                student_number: sid || `STU-${Date.now()}`,
                name: `${lr.firstName || ''} ${lr.lastName || ''}`.trim() || lr.fullName || lr.name || 'Student',
                email: lr.email || '',
                level: lr.level || 'JHS',
                grade: lr.grade || 'Grade 7',
                section: lr.section || 'N/A',
                strand: lr.strand || 'N/A',
                voucher: 'None',
                paid: 0,
                status: lr.status || (lr.active === false ? 'disabled' : 'active'),
                clearance: { Teacher: 'pending', Guidance: 'pending', Prefect: 'pending', Library: 'pending', Principal: 'pending', Accounting: 'pending', Registrar: 'pending' }
              });
            }
          }
        });
      } catch (e) {
        console.warn('Reading pa_registered_users error:', e);
      }
      return list;
    }

    function renderStudentAccounts() {
      const body = document.getElementById('studentAccountsBody');
      if (!body) return;

      const students = getAllRegisteredStudentsList();
      const totalCount = students.length;
      const activeCount = students.filter(s => s.status !== 'disabled' && s.status !== 'inactive').length;
      const disabledCount = students.filter(s => s.status === 'disabled' || s.status === 'inactive').length;

      const elTotal = document.getElementById('totalStudentAccountsCount');
      const elActive = document.getElementById('activeStudentAccountsCount');
      const elDisabled = document.getElementById('disabledStudentAccountsCount');
      const badgeStudent = document.getElementById('badgeStudentCount');
      if (elTotal) elTotal.textContent = totalCount;
      if (elActive) elActive.textContent = activeCount;
      if (elDisabled) elDisabled.textContent = disabledCount;
      if (badgeStudent) badgeStudent.textContent = totalCount;

      const searchVal = (document.getElementById('studentAccountSearchInput')?.value || '').trim().toLowerCase();
      const gradeVal = document.getElementById('studentAccountGradeFilter')?.value || 'all';
      const statusVal = document.getElementById('studentAccountStatusFilter')?.value || 'all';

      const filtered = students.filter(s => {
        if (gradeVal !== 'all' && s.grade !== gradeVal) return false;
        const isAct = s.status !== 'disabled' && s.status !== 'inactive';
        if (statusVal === 'active' && !isAct) return false;
        if (statusVal === 'disabled' && isAct) return false;
        if (searchVal) {
          const matchName = String(s.name || '').toLowerCase().includes(searchVal);
          const matchId = String(s.id || s.student_number || '').toLowerCase().includes(searchVal);
          const matchEmail = String(s.email || '').toLowerCase().includes(searchVal);
      ...
      if (filtered.length === 0) {
        body.innerHTML = `
          <tr>
            <td colspan="6" class="p-8 text-center text-academy-muted">
              <span class="material-symbols-rounded text-3xl text-slate-400 mb-1">person_search</span>
              <p class="font-extrabold text-sm text-academy-navy">No Student Accounts Found</p>
              <p class="text-xs mt-0.5">No students match your criteria. Try adjusting filters or click "+ Register / Add Student Account".</p>
            </td>
          </tr>
        `;
        return;
      ...
      body.innerHTML = filtered.map(s => {
        const isAct = s.status !== 'disabled' && s.status !== 'inactive';
        const strandDisplay = s.strand && s.strand !== 'N/A' ? ` (${s.strand})` : '';
        return `
          <tr class="hover:bg-academy-soft transition">
            <td class="px-4 py-3 font-mono font-bold text-academy-navy text-xs">${s.id || s.student_number || 'N/A'}</td>
            <td class="px-4 py-3 font-extrabold text-academy-navy">${s.name}</td>
            <td class="px-4 py-3 text-xs font-semibold text-slate-700">${s.grade || 'Grade 7'} - ${s.section || 'N/A'}${strandDisplay}</td>
            <td class="px-4 py-3 text-xs text-slate-600 font-mono">${s.email || 'No email registered'}</td>
            <td class="px-4 py-3">${pill(isAct ? 'Active' : 'Disabled', isAct ? 'green' : 'red')}</td>
            <td class="px-4 py-3">
              <div class="flex flex-wrap items-center gap-1.5">
                <button type="button" onclick="toggleStudentAccountStatus('${s.id}')"
                  class="px-2.5 py-1.5 rounded-lg ${isAct ? 'bg-amber-50 hover:bg-amber-100 text-amber-800 border border-amber-200/80' : 'bg-emerald-50 hover:bg-emerald-100 text-emerald-800 border border-emerald-200/80'} font-bold text-xs flex items-center gap-1 transition"
                  title="${isAct ? 'Disable student portal login' : 'Enable student portal login'}">
                  <span class="material-symbols-rounded text-[15px]">${isAct ? 'block' : 'check_circle'}</span>
                  ${isAct ? 'Disable Portal' : 'Enable Portal'}
                </button>
                <button type="button" onclick="openResetStudentPasswordModal('${s.id}')"
                  class="px-2.5 py-1.5 rounded-lg bg-sky-50 hover:bg-sky-100 text-sky-800 border border-sky-200/80 font-bold text-xs flex items-center gap-1 transition"
                  title="Reset student portal password">
                  <span class="material-symbols-rounded text-[15px]">key</span>
                  Reset Pass
                </button>
                <button type="button" onclick="openStudentAccountModal('${s.id}')"
                  class="px-2.5 py-1.5 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-800 border border-slate-200 font-bold text-xs flex items-center gap-1 transition"
                  title="Edit student account details">
                  <span class="material-symbols-rounded text-[15px]">edit</span>
                  Edit
                </button>
                <button type="button" onclick="deleteStudentAccount('${s.id}')"
                  class="px-2 py-1.5 rounded-lg bg-red-50 hover:bg-red-100 text-red-600 border border-red-200 font-bold text-xs flex items-center justify-center transition"
    ...
    async function syncStudentStatusChange(s, newStatus) {
      s.status = newStatus;
      s.active = (newStatus === 'active');

      const target = (appData.students || []).find(x => x.id === s.id || (s.email && String(x.email).toLowerCase() === String(s.email).toLowerCase()));
      if (target) {
      ...
      try {
        ...
        const idx = users.findIndex(u => (u.studentId && u.studentId === s.id) || (u.id && u.id === s.id) || (u.email && String(u.email).toLowerCase() === String(s.email).toLowerCase()));
        if (idx >= 0) {
          users[idx].status = newStatus;
          users[idx].active = (newStatus === 'active');
      ...
      if (window.paApi && window.paApi.client) {
        ...
        if (client) {
          try {
            await client.from('students').update({ status: newStatus }).or(`student_number.eq.${s.id},email.eq.${s.email}`);
            await client.from('profiles').update({ status: newStatus }).eq('email', s.email);
          } catch (err) {
            console.warn('Supabase student status update error:', err);
          }
        }
      }
    ...
    function toggleStudentAccountStatus(id) {
      const students = getAllRegisteredStudentsList();
      const s = students.find(x => x.id === id);
      if (!s) return;

      const isCurrentlyActive = s.status !== 'disabled' && s.status !== 'inactive';

      if (isCurrentlyActive) {
        showConfirmModal({
          title: 'Disable Student Portal Access',
          message: `Are you sure you want to disable student portal access for "${s.name}" (${s.id})? While disabled, this student will NOT be able to log in to view bills, make payments, or check clearance. (Note: Staff clearance head accounts are NOT affected).`,
          confirmText: 'Disable Student Portal',
          confirmClass: 'bg-amber-600 hover:bg-amber-700 text-white',
          onConfirm: async () => {
            await syncStudentStatusChange(s, 'disabled');
            showToast('Student Portal Disabled', `Portal access for ${s.name} has been disabled.`, 'warning');
          }
        });
      } else {
        showConfirmModal({
          title: 'Enable Student Portal Access',
          message: `Re-enable portal access for student "${s.name}" (${s.id})? The student will immediately be permitted to log in to the student portal.`,
          confirmText: 'Enable Student Portal',
          confirmClass: 'bg-emerald-600 hover:bg-emerald-700 text-white',
          onConfirm: async () => {
            await syncStudentStatusChange(s, 'active');
            showToast('Student Portal Enabled', `Portal access for ${s.name} has been restored.`, 'info');
          }
        });
      }
    }
    ...
    function openResetStudentPasswordModal(id) {
      ...
      formModalLabel.textContent = 'Student Account Security';
      formModalTitle.textContent = `Reset Password for ${s.name}`;
      formModalContent.innerHTML = `
        <form id="resetStudentPasswordForm" class="space-y-4">
          <div class="p-3.5 bg-sky-50 border border-sky-100 rounded-xl text-xs text-academy-navy space-y-1">
            <p><strong>Student:</strong> ${s.name} (${s.id})</p>
            <p><strong>Login Email:</strong> ${s.email || 'No email registered'}</p>
            <p class="text-academy-muted text-[11px] mt-1">This sets a new student portal login password. This operates independently from faculty and clearance accounts.</p>
          </div>
          <div>
            <label class="block text-xs font-extrabold text-academy-navy mb-1">New Portal Password</label>
            <div class="flex gap-2">
              <input id="newStudentPasswordInput" type="text" required placeholder="Enter new password (min 6 characters)" minlength="6"
                class="w-full px-4 py-3 rounded-xl border border-academy-border text-xs font-mono font-bold focus:outline-none focus:ring-2 focus:ring-academy-blue/20">
              <button type="button" onclick="generateRandomStudentPassword()"
                class="px-4 py-3 bg-slate-100 hover:bg-slate-200 border border-academy-border rounded-xl text-xs font-bold whitespace-nowrap text-academy-navy transition">
                Auto-Generate
              </button>
            </div>
          </div>
          <button class="w-full py-3 rounded-xl bg-academy-navy hover:bg-blue-900 text-white font-bold text-xs shadow-md transition">
            Save & Issue New Password
          </button>
        </form>
      `;

      document.getElementById('resetStudentPasswordForm').onsubmit = async (e) => {
        e.preventDefault();
        const newPass = document.getElementById('newStudentPasswordInput').value.trim();
        if (newPass.length < 6) {
        ...
        try {
          ...
          if (idx >= 0) {
            ...
            users[idx].tempPassword = newPass;
          } else {
            users.push({
              id: s.id,
              studentId: s.id,
        ...
        closeFormModal();
        showToast('Password Updated', `New password for ${s.name}: ${newPass}`, 'info');
      };
    ...
    function openStudentAccountModal(id = '') {
      const students = getAllRegisteredStudentsList();
      const s = id ? students.find(x => x.id === id) : null;

      formModalLabel.textContent = 'Student Portal Account';
      formModalTitle.textContent = s ? `Edit Student: ${s.name}` : 'Register New Student Account';

      const defaultGrade = s ? (s.grade || 'Grade 7') : 'Grade 7';
      const defaultLevel = s ? (s.level || inferLevelFromGrade(defaultGrade)) : 'JHS';
      const sections = SECTIONS_BY_GRADE[defaultGrade] || ['A'];

      formModalContent.innerHTML = `
        <form id="studentAccountForm" class="space-y-4">
          <input type="hidden" id="studentAccountOrigId" value="${s ? s.id : ''}">
          <div class="grid sm:grid-cols-2 gap-3">
            <div>
              <label class="block text-xs font-extrabold text-academy-navy mb-1">Student Full Name</label>
              <input id="studentAccountName" required value="${s ? s.name : ''}" placeholder="e.g. Juan Dela Cruz"
                class="w-full px-4 py-2.5 rounded-xl border border-academy-border text-xs font-semibold">
            </div>
            <div>
              <label class="block text-xs font-extrabold text-academy-navy mb-1">Student Number / ID</label>
              <input id="studentAccountId" required value="${s ? (s.id || s.student_number) : ''}" placeholder="e.g. 2026-0001"
                class="w-full px-4 py-2.5 rounded-xl border border-academy-border text-xs font-mono font-bold">
            </div>
          </div>

          <div class="grid sm:grid-cols-2 gap-3">
            <div>
              <label class="block text-xs font-extrabold text-academy-navy mb-1">Portal Login Email</label>
              <input id="studentAccountEmail" type="email" required value="${s ? s.email : ''}" placeholder="e.g. juan@student.edu"
                class="w-full px-4 py-2.5 rounded-xl border border-academy-border text-xs font-semibold">
            </div>
            <div>
              <label class="block text-xs font-extrabold text-academy-navy mb-1">Portal Access Status</label>
              <select id="studentAccountStatus" class="w-full px-4 py-2.5 rounded-xl border border-academy-border text-xs font-bold bg-white">
                <option value="active" ${(s && (s.status === 'active' || !s.status)) ? 'selected' : ''}>Active (Portal Access Enabled)</option>
                <option value="disabled" ${(s && (s.status === 'disabled' || s.status === 'inactive')) ? 'selected' : ''}>Disabled (Portal Access Suspended)</option>
              </select>
            </div>
          </div>
          ...
          <div class="grid sm:grid-cols-3 gap-3">
            <div>
              <label class="block text-xs font-extrabold text-academy-navy mb-1">Grade Level</label>
              <select id="studentAccountGrade" onchange="updateStudentModalSections()"
                class="w-full px-4 py-2.5 rounded-xl border border-academy-border text-xs font-bold bg-white">
                <option value="Grade 7" ${defaultGrade === 'Grade 7' ? 'selected' : ''}>Grade 7</option>
                <option value="Grade 8" ${defaultGrade === 'Grade 8' ? 'selected' : ''}>Grade 8</option>
                <option value="Grade 9" ${defaultGrade === 'Grade 9' ? 'selected' : ''}>Grade 9</option>
                <option value="Grade 10" ${defaultGrade === 'Grade 10' ? 'selected' : ''}>Grade 10</option>
                <option value="Grade 11" ${defaultGrade === 'Grade 11' ? 'selected' : ''}>Grade 11</option>
                <option value="Grade 12" ${defaultGrade === 'Grade 12' ? 'selected' : ''}>Grade 12</option>
              </select>
            </div>
            <div>
              <label class="block text-xs font-extrabold text-academy-navy mb-1">Section</label>
              <select id="studentAccountSection"
                class="w-full px-4 py-2.5 rounded-xl border border-academy-border text-xs font-bold bg-white">
                ${sections.map(sec => `<option value="${sec}" ${(s && s.section === sec) ? 'selected' : ''}>${sec}</option>`).join('')}
              </select>
            </div>
            <div>
              <label class="block text-xs font-extrabold text-academy-navy mb-1">Strand (SHS)</label>
              <select id="studentAccountStrand"
                class="w-full px-4 py-2.5 rounded-xl border border-academy-border text-xs font-bold bg-white">
                <option value="N/A" ${(s && s.strand === 'N/A') ? 'selected' : ''}>N/A (JHS)</option>
                <option value="STEM" ${(s && s.strand === 'STEM') ? 'selected' : ''}>STEM</option>
                <option value="ABM" ${(s && s.strand === 'ABM') ? 'selected' : ''}>ABM</option>
                <option value="HUMSS" ${(s && s.strand === 'HUMSS') ? 'selected' : ''}>HUMSS</option>
                <option value="GAS" ${(s && s.strand === 'GAS') ? 'selected' : ''}>GAS</option>
                <option value="TVL" ${(s && s.strand === 'TVL') ? 'selected' : ''}>TVL</option>
              </select>
            </div>
          </div>

          <div>
            <label class="block text-xs font-extrabold text-academy-navy mb-1">
              ${s ? 'New Password (leave blank to keep current)' : 'Initial Portal Password'}
            </label>
            <input id="studentAccountPassword" type="text" placeholder="${s ? 'Leave empty to keep unchanged' : 'e.g. StudentPass2026'}"
              ${s ? '' : 'required'} minlength="6"
              class="w-full px-4 py-2.5 rounded-xl border border-academy-border text-xs font-mono font-bold">
          </div>

          <button type="submit" class="w-full py-3 rounded-xl bg-academy-navy hover:bg-blue-900 text-white font-bold text-xs shadow-md transition">
            ${s ? 'Save Student Changes' : 'Register Student Account'}
          </button>
        </form>
      `;

      document.getElementById('studentAccountForm').onsubmit = async (e) => {
        e.preventDefault();
        const origId = document.getElementById('studentAccountOrigId').value;
        const name = document.getElementById('studentAccountName').value.trim();
        const newId = document.getElementById('studentAccountId').value.trim();
        const email = document.getElementById('studentAccountEmail').value.trim();
        const status = document.getElementById('studentAccountStatus').value;
        const grade = document.getElementById('studentAccountGrade').value;
        const section = document.getElementById('studentAccountSection').value;
        const strand = ['Grade 11', 'Grade 12'].includes(grade) ? document.getElementById('studentAccountStrand').value : 'N/A';
        const level = ['Grade 11', 'Grade 12'].includes(grade) ? 'SHS' : 'JHS';
        const pass = document.getElementById('studentAccountPassword').value.trim();

        if (origId) {
          // Editing existing student
          const target = appData.students.find(x => x.id === origId);
          if (target) {
            target.id = newId;
            target.student_number = newId;
            target.name = name;
            target.email = email;
            target.grade = grade;
          ...
          try {
            const users = JSON.parse(localStorage.getItem('pa_registered_users') || '[]');
            const uIdx = users.findIndex(u => (u.studentId && u.studentId === origId) || (u.id && u.id === origId) || (u.email && String(u.email).toLowerCase() === email.toLowerCase()));
            if (uIdx >= 0) {
              users[uIdx].studentId = newId;
              users[uIdx].id = newId;
              users[uIdx].name = name;
              users[uIdx].email = email;
              users[uIdx].grade = grade;
              users[uIdx].section = section;
              users[uIdx].strand = strand;
              users[uIdx].level = level;
              users[uIdx].status = status;
              users[uIdx].active = (status === 'active');
              if (pass) {
                users[uIdx].password = pass;
                users[uIdx].tempPassword = pass;
              }
              localStorage.setItem('pa_registered_users', JSON.stringify(users));
            }
          } catch (err) {}
            ...
            if (client) {
              try {
                await client.from('students').update({
                  ...
                  status: status
                }).or(`student_number.eq.${origId},email.eq.${email}`);
                await client.from('profiles').update({ full_name: name, status: status }).eq('email', email);
              } catch (err) {}
            }
          }
          ...
          closeFormModal();
          renderAll();
          showToast('Student Updated', `Updated account information for ${name}.`);
        } else {
          ...
          // Supabase registration if paApi available
          if (window.paApi && typeof window.paApi.registerStudent === 'function' && pass) {
            try {
              await window.paApi.registerStudent({
                firstName: name.split(' ')[0],
                lastName: name.split(' ').slice(1).join(' '),
                studentId: newId,
                email: email,
              ...
              console.warn('registerStudent call notice:', err);
            }
          }
          ...
          closeFormModal();
          renderAll();
          showToast('Student Created', `Successfully registered student ${name} (${newId}).`);
        }
      };
    ...
    function updateStudentModalSections() {
      const grade = document.getElementById('studentAccountGrade')?.value || 'Grade 7';
      const sectionSelect = document.getElementById('studentAccountSection');
      const strandSelect = document.getElementById('studentAccountStrand');
      if (sectionSelect) {
        const list = SECTIONS_BY_GRADE[grade] || ['A'];
        sectionSelect.innerHTML = list.map(sec => `<option value="${sec}">${sec}</option>`).join('');
      }
      if (strandSelect) {
        const isShs = ['Grade 11', 'Grade 12'].includes(grade);
        strandSelect.disabled = !isShs;
        if (!isShs) strandSelect.value = 'N/A';
      }
    }
    ...
    function deleteStudentAccount(id) {
      ...
      showConfirmModal({
        title: 'Delete Student Account',
        message: `Are you sure you want to permanently delete the student account for "${s.name}" (${s.id})? This will remove portal credentials and local records. This action cannot be undone.`,
        ...
        onConfirm: async () => {
          ...
          try {
            const users = JSON.parse(localStorage.getItem('pa_registered_users') || '[]');
            const updated = users.filter(u => u.studentId !== id && u.id !== id && String(u.email || '').toLowerCase() !== String(s.email || '').toLowerCase());
            localStorage.setItem('pa_registered_users', JSON.stringify(updated));
          } catch (err) {}
          ...
          if (window.paApi && window.paApi.client) {
            ...
            if (client) {
              try {
                await client.from('students').delete().or(`student_number.eq.${id},email.eq.${s.email}`);
                await client.from('profiles').delete().eq('email', s.email);
              } catch (err) {}
            }
          }
          ...
          saveData();
          renderAll();
          showToast('Student Account Deleted', `Removed student account for ${s.name}.`);
        }
      });
    }
        ...
        try {
          ...
          if (state) {
            ...
            if (state.installmentTemplate && state.installmentTemplate.length > 0) {
              const cleanInst = state.installmentTemplate.filter(p => !['ins-downpayment-2026', 'ins-q2-2026', 'ins-q3-2027', 'ins-q4-2027'].includes(p.id));
              appData.installmentTemplate = cleanInst;
              localStorage.setItem('pa_installment_templates', JSON.stringify(appData.installmentTemplate));
            }
